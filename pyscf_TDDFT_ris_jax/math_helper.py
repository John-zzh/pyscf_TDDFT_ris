...
import scipy
import time
from pyscf_TDDFT_ris_jax import parameter
# import os, psutil
jnp.set_printoptions(linewidth=250, threshold=jnp.inf, precision=3)
...
def TDA_diag_initial_guess(V_holder, N_states, hdiag):
    ...
    '''
    hdiag = jnp.array(hdiag).reshape(-1,)
    Dsort = hdiag.argsort()
    # print('hdiag[Dsort[:N_states]]',hdiag[Dsort[:N_states]]*parameter.Hartree_to_eV)
    # print('type(V_holder)', type(V_holder))
    # V_holder = np.array(V_holder)
    # for j in range(N_states):
    #     V_holder[Dsort[j], j] = 1.0
        # V_holder = V_holder.at[Dsort[j], j].set(1.0)
    V_holder = V_holder.at[(Dsort[:N_states], jnp.arange(N_states))].set(1.0)
    # V_holder = jnp.array(V_holder)
    return V_holder
...
def TDA_diag_preconditioner(residual, sub_eigenvalue, hdiag ):
    ...
    N_states = jnp.shape(residual)[1]
    t = 1e-8
    # D = jnp.repeat(hdiag.reshape(-1,1), N_states, axis=1) - sub_eigenvalue
    # '''
    # force all small values not in [-t,t]
    # '''
    # D = jnp.where( abs(D) < t, jnp.sign(D)*t, D)

    D = np.repeat(hdiag.reshape(-1,1), N_states, axis=1) - sub_eigenvalue
    '''
    force all small values not in [-t,t]
    '''
    D = np.where( abs(D) < t, jnp.sign(D)*t, D)
...
def TDDFT_diag_preconditioner(R_x, R_y, omega, hdiag):
    ...
    t = 1e-14
    d = jnp.repeat(hdiag.reshape(-1,1), N_states, axis=1)

    D_x = d - omega
    D_x = jnp.where(abs(D_x) < t, jnp.sign(D_x)*t, D_x)
    D_x_inv = D_x**-1

    D_y = d + omega
    D_y = jnp.where(abs(D_y) < t, jnp.sign(D_y)*t, D_y)
    D_y_inv = D_y**-1
...
def commutator(A,B):
    commu = jnp.dot(A,B) - jnp.dot(B,A)
    return commu

def cond_number(A):
    s,u = jnp.linalg.eig(A)
    s = abs(s)
    cond = max(s)/min(s)
    return cond
...
def matrix_power(S,a):
    '''X == S^a'''
    s,ket = jnp.linalg.eigh(S)
    s = s**a
    X = jnp.dot(ket*s,ket.T)
    return X
...
#     '''
#     if A.shape[1] != 0:
#         projections_coeff = jnp.dot(A.T, bvec)
#         bvec -= jnp.dot(A, projections_coeff)
#     return bvec

@jax.jit
def Gram_Schmidt_bvec(V, vec):
    # Placeholder for the Gram_Schmidt_bvec function.
...
def utriangle_symmetrize(A):
    upper = jnp.triu_indices(n=A.shape[0], k=1)
    lower = (upper[1], upper[0])
    # A[lower] = A[upper]
    A = A.at[lower].set(A[upper])
    return A
...
def gen_VW(sub_A_holder, V_holder, W_holder, size_old, size_new, symmetry = True, up_triangle = False):
    ...
    # sub_A_holder[:size_new,size_old:size_new] = jnp.dot(V_current.T, W_new)
    sub_A_holder = sub_A_holder.at[:size_new,size_old:size_new].set(jnp.dot(V_current.T, W_new))
    ...
    if symmetry == True:
        ...
        if up_triangle == False:
            ...
            sub_A_holder[size_old:size_new,:size_old] = jnp.dot(V_new.T, W_old)
...
def gen_VP(sub_P_holder, V_holder, P, size_old, size_new):
    ...
    '''
    V_new = V_holder[:,size_old:size_new]
    sub_P_holder[size_old:size_new,:] = jnp.dot(V_new.T, P)
    return sub_P_holder
...
def Gram_Schmidt_fill_holder(V, count, vecs, double = True):
    ...
    '''
    nvec = jnp.shape(vecs)[1]
    for j in range(nvec):
        vec = vecs[:, j].reshape(-1,1)
        V_tmp = V[:, :count]
        vec = Gram_Schmidt_bvec(V_tmp, vec)   #single orthonormalize
        if double == True:
            vec = Gram_Schmidt_bvec(V_tmp, vec)   #double orthonormalize
...
def S_symmetry_orthogonal(x,y):
    ...
    '''
    x_p_y = x + y
    x_p_y_norm = jnp.linalg.norm(x_p_y)

    x_m_y = x - y
    x_m_y_norm = jnp.linalg.norm(x_m_y)
...
def VW_Gram_Schmidt_fill_holder(V_holder, W_holder, m, X_new, Y_new, double = False):
    ...
    for j in range(0, nvec):
        ...
        xy_norm = (jnp.dot(x_tmp.T, x_tmp)+jnp.dot(y_tmp.T, y_tmp))**0.5
...
def TDDFT_subspace_eigen_solver2(a, b, sigma, pi, k):
    ...
    # L = jnp.dot(P_permutation, L)

    # L_inv = jnp.linalg.inv(L)
    # U_inv = jnp.linalg.inv(U)

    L_inv = jnp.linalg.cholesky(jnp.linalg.inv(s_m_p))
    U_inv = L_inv.T
    ''' a ̃−b ̃= U^-T d^−1/2 (a−b) d^-1/2 U^-1 = GG^T '''
    dambd =  d_mh.reshape(-1,1)*(a-b)*d_mh.reshape(1,-1)
    GGT = jnp.linalg.multi_dot([U_inv.T, dambd, U_inv])
    ...
    M = jnp.linalg.multi_dot([G.T, L_inv, dapbd, L_inv.T, G])

    omega2, Z = jnp.linalg.eigh(M)
    omega = (omega2**0.5)[:k]
    Z = Z[:,:k]
    ...
    x_p_y = d_mh.reshape(-1,1)\
            *jnp.linalg.multi_dot([L_inv.T, G, Z])\
            *(jnp.array(omega)**-0.5).reshape(1,-1)

    x_m_y = d_mh.reshape(-1,1)\
            *jnp.linalg.multi_dot([U_inv, G_inv.T, Z])\
            *(jnp.array(omega)**0.5).reshape(1,-1)
...
def TDDFT_subspace_eigen_solver3(a, b, sigma, pi, k):
    ...
    M = jnp.linalg.multi_dot([B_neg_tmp, A, B_neg_tmp])
...
def TDDFT_subspace_eigen_solver(a, b, sigma, pi, k):
    ...
    M = jnp.linalg.multi_dot([A_neg_tmp, B, A_neg_tmp])
    ...
    T = jnp.dot(A_neg_tmp, Z)
    x = T[:half_size,:]
    y = T[half_size:,:]

    # xy_norm_check = jnp.linalg.norm( (jnp.dot(x.T,x) - jnp.dot(y.T,y)) -jnp.eye(k) )
