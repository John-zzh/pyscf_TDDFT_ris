...
from pyscf_TDDFT_ris_jax import math_helper
...
def Davidson(matrix_vector_product,
    ...
    V_holder = jnp.zeros((A_size, max_N_mv),dtype=jnp.float32 if single else jnp.float64)
    W_holder = jnp.zeros_like(V_holder)
    sub_A_holder = jnp.zeros((max_N_mv,max_N_mv),dtype=jnp.float32 if single else jnp.float64)
    '''
    ...
    # V_previous = math_helper.TDA_diag_initial_guess(N_states=size_new, hdiag=hdiag)
    ...
    for ii in range(max_iter):
        ...
        mv_product = matrix_vector_product(V_holder[:,size_old:size_new])
        W_holder = W_holder.at[:, size_old:size_new].set(mv_product)
        # if W_previous:
        #     W_currret = jnp.vstack(W_previous, mv_product)
        MV_end = time.time()
        ...
        # sub_A = jnp.dot(V_holder[:,:size_new].T, W_holder[:, :size_new])
        ...
        sub_eigenvalue, sub_eigenket = jnp.linalg.eigh(sub_A)
        ...
        # print('energies', sub_eigenvalue*parameter.Hartree_to_eV)


        full_cost_start = time.time()
        full_guess = jnp.dot(V_holder[:,:size_new], sub_eigenket)
        full_cost_end = time.time()
        full_cost += full_cost_end - full_cost_start

        AV = jnp.dot(W_holder[:,:size_new], sub_eigenket)
        residual = AV - full_guess * sub_eigenvalue

        # r_norms = jnp.linalg.norm(residual, axis=0).tolist()
        r_norms = jnp.linalg.norm(residual, axis=0)
        max_norm = jnp.max(r_norms)
        # print(r_norms)
        ...
        index = jnp.where(r_norms > conv_tol)[0]
        # print('index', index)
...
def Davidson_Casida(matrix_vector_product,
    ...
    U1_holder = jnp.zeros_like(V_holder)
    U2_holder = jnp.zeros_like(V_holder)

    VU1_holder = jnp.zeros((max_N_mv,max_N_mv),dtype=jnp.float32 if single else jnp.float64)
    VU2_holder = jnp.zeros_like(VU1_holder)
    WU1_holder = jnp.zeros_like(VU1_holder)
    WU2_holder = jnp.zeros_like(VU1_holder)

    VV_holder = jnp.zeros_like(VU1_holder)
    VW_holder = jnp.zeros_like(VU1_holder)
    WW_holder = jnp.zeros_like(VU1_holder)
