...
from pyscf_TDDFT_ris_jax import parameter, eigen_solver, math_helper, spectralib
print('jax.devices()', jax.devices())

jnp.set_printoptions(linewidth=250, threshold=jnp.inf)
jax.config.update('jax_default_matmul_precision', 'highest')
# jaxeinsum = jax.numpy.einsum
# einsum = numpy.einsum
...
class TDDFT_ris(object):
    ...
    def gen_eri2c_eri3c(self, mol, auxmol, omega=0):
        ...
        eri3c = jnp.array(eri3c)
        # print('mol.nbas =', mol.nbas)
        if self.single:
            eri2c = eri2c.astype(jnp.float32)
            eri3c = eri3c.astype(jnp.float32)    
    ...
    def gen_uvQL(self, eri3c, eri2c):
        ...
        Lower = jnp.linalg.cholesky(jnp.linalg.inv(eri2c))
        # e, v = jnp.linalg.eigh(jnp.linalg.inv(eri2c))
        # L = jnp.dot(v, jnp.sqrt(jnp.diag(e)))
    ...
    def gen_B(self, uvQL, n_occ, mo_coeff, calc=None):
        ...
        if self.single == True:
            mo_coeff = mo_coeff.astype(jnp.float32)
        tmp = einsum("vq,uvP->uqP", mo_coeff, uvQL)

        B_raw = einsum("up,uqP->pqP", mo_coeff, tmp)
        ...
        if calc == 'both' or calc == 'exchange_only':
            ...
            B_ij = B_raw[:n_occ,:n_occ,:]
            B_ab = B_raw[n_occ:,n_occ:,:]

            return jnp.array(B_ia), jnp.array(B_ij), jnp.array(B_ab)
    ...
    def gen_hdiag_fly(self, mo_energy, n_occ, n_vir, sqrt=False):
        ...
        delta_hdiag = jnp.repeat(vir, n_occ, axis=0) - jnp.repeat(occ, n_vir, axis=1)
        # delta_hdiag_numpy = np.repeat(vir, n_occ, axis=0) - jnp.repeat(occ, n_vir, axis=1)
        # print("hdiagdiff", np.linalg.norm(delta_hdiag - delta_hdiag_numpy))

        hdiag = delta_hdiag.reshape(n_occ*n_vir)
        # print(hdiag)
        # print('delta_hdiag[-1,0]', delta_hdiag[-1,0]*parameter.Hartree_to_eV)
        # Hdiag = jnp.vstack((hdiag, hdiag))
        ...
        if sqrt == False:
            ...
            '''
            delta_hdiag_sqrt = jnp.sqrt(delta_hdiag)
            hdiag_sq = hdiag**2
    ...
    def gen_UKS_TDA_mv(self):
        ...
        hdiag = jnp.vstack((hdiag_a.reshape(-1,1), hdiag_b.reshape(-1,1))).reshape(-1)
    ...
    def gen_UKS_TDDFT_mv(self):
        ...
        if a_x != 0:
            ...
            def UKS_TDDFT_hybrid_mv(X,Y):
                ...
                ApB_XpY = jnp.vstack((ApB_XpY_alpha, ApB_XpY_beta))

                AmB_XmY_alpha = AmB_XmY_aa.reshape(A_aa_size,-1)
                AmB_XmY_beta  = AmB_XmY_bb.reshape(A_bb_size,-1)
                AmB_XmY = jnp.vstack((AmB_XmY_alpha, AmB_XmY_beta))
            ...
            hdiag_sq = jnp.vstack((hdiag_a_sq.reshape(-1,1), hdiag_b_sq.reshape(-1,1))).reshape(-1)
    ...
    def kernel_TDDFT(self):     
        ...
        if self.a_x != 0:
            ...
            # print('check norm of Z', jnp.linalg.norm(jnp.dot(Z.T,Z) - jnp.eye(Z.shape[1])))
            energies = energies_sq**0.5
            Z = Z*energies**0.5

            X, Y = math_helper.XmY_2_XY(Z=Z, AmB_sq=hdiag_sq, omega=energies)

        XY_norm_check = jnp.linalg.norm( (jnp.dot(X.T,X) - jnp.dot(Y.T,Y)) -jnp.eye(self.nroots) )
        print('check norm of X^TX - Y^YY - I = {:.2e}'.format(XY_norm_check))

        energies = energies*parameter.Hartree_to_eV

        oscillator_strength = spectralib.get_spectra(energies=energies, 
                                                    transition_vector= np.array(X+Y), 
                                                    X = np.array(X/(2**0.5)),
                                                    Y = np.array(Y/(2**0.5)),
                                                    P=np.array(P), 
